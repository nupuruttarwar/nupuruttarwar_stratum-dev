...
// P4OVS_CHANGES: TransceiverEvents
...
/* static */
constexpr int IpdkChassisManager::kMaxPortStatusEventDepth;
/* static */
constexpr int IpdkChassisManager::kMaxXcvrEventDepth;

IpdkChassisManager::IpdkChassisManager(OperationMode mode,
                                       PhalInterface* phal_interface,
                                       TdiSdeInterface* sde_interface)
    : mode_(mode),
      initialized_(false),
#ifndef P4OVS_CHANGES
      // P4OVS_CHANGES: PortStatusEvents
...
IpdkChassisManager::IpdkChassisManager()
...
IpdkChassisManager::~IpdkChassisManager() = default;

#ifdef P4OVS_CHANGES

// P4OVS_CHANGES: GnmiPortConfig
// P4OVS_CHANGES(dgf): Renamed from ValidateOneTimeConfig()

// Determines whether the specified port configuration parameter has
// already been set.
bool IpdkChassisManager::PortParamAlreadySet(
...
// P4OVS_CHANGES: GnmiPortConfig
// P4OVS_CHANGES(dgf): Renamed from ValidateAndAdd().

// Sets the value of a port configuration parameter.
::util::Status IpdkChassisManager::SetPortParam(
    uint64 node_id, uint32 port_id,
...
::util::Status IpdkChassisManager::AddPortHelper(
...
#if defined(P4OVS_CHANGES) && !defined(TOFINO_TARGET)
  // P4OVS_CHANGES: GnmiPortConfig
...
#ifndef P4OVS_CHANGES
  // P4OVS_CHANGES: PortShapingConfig
  RETURN_IF_ERROR(
      sde_interface_->EnablePortShaping(unit, sdk_port_id, TRI_STATE_FALSE));
#endif

  return ::util::OkStatus();
}

::util::Status IpdkChassisManager::UpdatePortHelper(
    uint64 node_id, int unit, uint32 sdk_port_id,
    const SingletonPort& singleton_port /* desired config */,
...
::util::Status IpdkChassisManager::PushChassisConfig(
    const ChassisConfig& config) {
#ifndef P4OVS_CHANGES
  // P4OVS_CHANGES: EventWriters
  ...
  if (config.has_vendor_config() &&
    ...
    for (const auto& key : node_id_to_deflect_on_drop_configs) {
      ...
      for (const auto& drop_target : deflect_config.drop_targets()) {
        ...
        RETURN_IF_ERROR(sde_interface_->SetDeflectOnDropDestination(
...
#ifndef P4OVS_CHANGES
// P4OVS_CHANGES: PortShapingConfig
::util::Status IpdkChassisManager::ApplyPortShapingConfig(
    uint64 node_id, int unit, uint32 sdk_port_id,
    const TofinoConfig::BfPortShapingConfig::BfPerPortShapingConfig&
        shaping_config) {
  switch (shaping_config.shaping_case()) {
    case TofinoConfig::BfPortShapingConfig::BfPerPortShapingConfig::
        kPacketShaping:
      RETURN_IF_ERROR(sde_interface_->SetPortShapingRate(
  ...
  RETURN_IF_ERROR(
      sde_interface_->EnablePortShaping(unit, sdk_port_id, TRI_STATE_TRUE));
...
::util::Status IpdkChassisManager::VerifyChassisConfig(
    const ChassisConfig& config) {
...
::util::Status IpdkChassisManager::RegisterEventNotifyWriter(
...
::util::Status IpdkChassisManager::UnregisterEventNotifyWriter() {
  absl::WriterMutexLock l(&gnmi_event_lock_);
  gnmi_event_writer_ = nullptr;
  return ::util::OkStatus();
}

::util::StatusOr<const IpdkChassisManager::PortConfig*>
IpdkChassisManager::GetPortConfig(uint64 node_id, uint32 port_id) const {
...
::util::StatusOr<uint32> IpdkChassisManager::GetSdkPortId(
...
::util::StatusOr<DataResponse> IpdkChassisManager::GetPortData(
...
#ifndef P4OVS_CHANGES
      // P4OVS_CHANGES: FrontPanelPortInfo
      RETURN_IF_ERROR(
...
::util::StatusOr<PortState> IpdkChassisManager::GetPortState(
    uint64 node_id, uint32 port_id) const {
  if (!initialized_) {
    return MAKE_ERROR(ERR_NOT_INITIALIZED) << "Not initialized!";
  }
...
::util::StatusOr<absl::Time> IpdkChassisManager::GetPortTimeLastChanged(
    uint64 node_id, uint32 port_id) {
  if (!initialized_) {
    return MAKE_ERROR(ERR_NOT_INITIALIZED) << "Not initialized!";
  }
...
::util::Status IpdkChassisManager::GetPortCounters(
...
::util::StatusOr<std::map<uint64, int>> IpdkChassisManager::GetNodeIdToUnitMap()
    const {
  if (!initialized_) {
    return MAKE_ERROR(ERR_NOT_INITIALIZED) << "Not initialized!";
  }
...
::util::Status IpdkChassisManager::ReplayPortsConfig(uint64 node_id) {
  if (!initialized_) {
    return MAKE_ERROR(ERR_NOT_INITIALIZED) << "Not initialized!";
  }
  ASSIGN_OR_RETURN(auto unit, GetUnitFromNodeId(node_id));
  ...
  for (const auto& drop_target :
    ...
    switch (drop_target.port_type_case()) {
      case TofinoConfig::DeflectOnPacketDropConfig::DropTarget::kPort: {
        ASSIGN_OR_RETURN(
            sdk_port_id, GetSdkPortId(node_id, drop_target.port()));
        break;
      }
      case TofinoConfig::DeflectOnPacketDropConfig::DropTarget::kSdkPort: {
        sdk_port_id = drop_target.sdk_port();
...
#ifndef P4OVS_CHANGES
// P4OVS_CONFIG: FrontPanelPortInfo
::util::Status IpdkChassisManager::GetFrontPanelPortInfo(
...
std::unique_ptr<IpdkChassisManager> IpdkChassisManager::CreateInstance(
    OperationMode mode, PhalInterface* phal_interface,
    TdiSdeInterface* sde_interface) {
  return absl::WrapUnique(
      new IpdkChassisManager(mode, phal_interface, sde_interface));
}

#ifndef P4OVS_CHANGES

void IpdkChassisManager::SendPortOperStateGnmiEvent(
...
// P4OVS_CHANGES: PortStatusEvents
void* IpdkChassisManager::PortStatusEventHandlerThreadFunc(void* arg) {
  CHECK(arg != nullptr);
  // Retrieve arguments.
...
// P4OVS_CHANGES: PortStatusEvents
void IpdkChassisManager::ReadPortStatusEvents(
...
// P4OVS_CHANGES: PortStatusEvents
void IpdkChassisManager::PortStatusEventHandler(
...
// P4OVS_CHANGES: TransceiverEvents
void* IpdkChassisManager::TransceiverEventHandlerThreadFunc(void* arg) {
  CHECK(arg != nullptr);
  // Retrieve arguments.
...
// P4OVS_CHANGES: TransceiverEvents
void IpdkChassisManager::ReadTransceiverEvents(
...
// P4OVS_CHANGES: TransceiverEvents
void IpdkChassisManager::TransceiverEventHandler(
    int slot, int port, HwState new_state) {
  absl::WriterMutexLock l(&chassis_lock);
...
// P4OVS_CHANGES: EventWriters
::util::Status IpdkChassisManager::RegisterEventWriters() {
  if (initialized_) {
  ...
  if (!port_status_event_channel_) {
    ...
    // Create and hand off Writer to the SdeInterface.
    auto writer =
        ChannelWriter<PortStatusEvent>::Create(port_status_event_channel_);
    RETURN_IF_ERROR(
        sde_interface_->RegisterPortStatusEventWriter(std::move(writer)));
...
// P4OVS_CHANGES: EventWriters
::util::Status IpdkChassisManager::UnregisterEventWriters() {
  absl::WriterMutexLock l(&chassis_lock);
  ::util::Status status = ::util::OkStatus();
  // Unregister the linkscan and transceiver module event Writers.
  APPEND_STATUS_IF_ERROR(status,
                         sde_interface_->UnregisterPortStatusEventWriter());
  // Close Channel.
...
::util::StatusOr<int> IpdkChassisManager::GetUnitFromNodeId(
...
void IpdkChassisManager::CleanupInternalState() {
...
::util::Status IpdkChassisManager::Shutdown() {
  ::util::Status status = ::util::OkStatus();
  {
    absl::ReaderMutexLock l(&chassis_lock);
