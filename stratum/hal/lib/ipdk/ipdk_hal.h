...
#ifndef STRATUM_HAL_LIB_IPDK_IPDK_HAL_H_
#define STRATUM_HAL_LIB_IPDK_IPDK_HAL_H_
...
// Class 'IpdkHal' is nothing but a wrapper around all the HAL services, which
...
class IpdkHal final {
 public:
  ...
  // the instance.
  static IpdkHal* CreateSingleton(OperationMode mode,
  ...
  static IpdkHal* GetSingleton() LOCKS_EXCLUDED(init_lock_);

  // IpdkHal is neither copyable nor movable.
  IpdkHal(const IpdkHal&) = delete;
  IpdkHal& operator=(const IpdkHal&) = delete;

  // Pipe file descriptors used to deliver signals from the handler to IpdkHal.
 ...
 private:
  ...
  // class.
  IpdkHal(OperationMode mode, SwitchInterface* switch_interface,
          AuthPolicyChecker* auth_policy_checker);
  ...
  static IpdkHal* singleton_ GUARDED_BY(init_lock_);
};

}  // namespace hal
}  // namespace stratum

#endif  // STRATUM_HAL_LIB_IPDK_IPDK_HAL_H_
