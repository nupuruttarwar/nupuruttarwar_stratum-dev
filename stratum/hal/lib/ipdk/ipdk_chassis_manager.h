...
#ifndef STRATUM_HAL_LIB_IPDK_IPDK_CHASSIS_MANAGER_H_
#define STRATUM_HAL_LIB_IPDK_IPDK_CHASSIS_MANAGER_H_
...
class IpdkChassisManager {
 public:
  virtual ~IpdkChassisManager();
  ...
  static std::unique_ptr<IpdkChassisManager> CreateInstance(
  ...
  bool PortParamAlreadySet(uint64 node_id, uint32 port_id,
  ...
  // IpdkChassisManager is neither copyable nor movable.
  IpdkChassisManager(const IpdkChassisManager&) = delete;
  IpdkChassisManager& operator=(const IpdkChassisManager&) = delete;
  IpdkChassisManager(IpdkChassisManager&&) = delete;
  IpdkChassisManager& operator=(IpdkChassisManager&&) = delete;
 ...
 private:
  ...
  // class.
  IpdkChassisManager(OperationMode mode, PhalInterface* phal_interface,
                     TdiSdeInterface* sde_interface);
  ...
  // first accesses the internal structures of a class below IpdkChassisManager
  ...
  // IpdkChassisManager as this may result in deadlock.
  ...
  friend class IpdkChassisManagerTest;
};
...
#endif  // STRATUM_HAL_LIB_IPDK_IPDK_CHASSIS_MANAGER_H_
