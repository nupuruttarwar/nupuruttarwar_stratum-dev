...
#include "stratum/hal/lib/ipdk/ipdk_chassis_manager_mock.h"
#include "stratum/hal/lib/ipdk/ipdk_node_mock.h"
...
class IpdkSwitchTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    chassis_manager_mock_ = absl::make_unique<NiceMock<IpdkChassisManagerMock>>();
    node_mock_ = absl::make_unique<NiceMock<IpdkNodeMock>>();
    unit_to_ipdk_node_mock_[kUnit] = node_mock_.get();
    switch_ = IpdkSwitch::CreateInstance(
        phal_mock_.get(),
  ...
  std::unique_ptr<IpdkChassisManagerMock> chassis_manager_mock_;
  std::unique_ptr<IpdkNodeMock> node_mock_;
  std::map<int, IpdkNode*> unit_to_ipdk_node_mock_;
  std::unique_ptr<IpdkSwitch> switch_;
};

TEST_F(IpdkSwitchTest, PushChassisConfigSucceeds) {
    PushChassisConfigSuccessfully();
}

TEST_F(IpdkSwitchTest, PushChassisConfigFailsWhenNodePushFails) {
  ChassisConfig config;
  config.add_nodes()->set_id(kNodeId);
...
TEST_F(IpdkSwitchTest, VerifyChassisConfigSucceeds) {
  ChassisConfig config;
  config.add_nodes()->set_id(kNodeId);
  EXPECT_OK(switch_->VerifyChassisConfig(config));
}

TEST_F(IpdkSwitchTest, ShutdownSucceeds) {
...
TEST_F(IpdkSwitchTest, ShutdownFailsWhenSomeManagerShutdownFails) {
  EXPECT_CALL(*node_mock_, Shutdown()).WillOnce(Return(::util::OkStatus()));
  EXPECT_CALL(*chassis_manager_mock_, Shutdown())
      .WillOnce(Return(DefaultError()));
  EXPECT_CALL(*phal_mock_, Shutdown()).WillOnce(Return(::util::OkStatus()));
...
TEST_F(IpdkSwitchTest, PushForwardingPipelineConfigSucceeds) {
  PushChassisConfigSuccessfully();
...
// When IpdkSwitchTest fails to push a forwarding config during
// PushForwardingPipelineConfig(), it should fail immediately.
TEST_F(IpdkSwitchTest, PushForwardingPipelineConfigFailsWhenPushFails) {
  PushChassisConfigSuccessfully();
...
TEST_F(IpdkSwitchTest, VerifyForwardingPipelineConfigSucceeds) {
  PushChassisConfigSuccessfully();
...
TEST_F(IpdkSwitchTest, RegisterEventNotifyWriterTest) {
  ...
  // Successful IpdkChassisManager registration.
  EXPECT_OK(switch_->RegisterEventNotifyWriter(writer));
  // Failed IpdkChassisManager registration.
...
#if 0
// No GetPortState()
TEST_F(IpdkSwitchTest, GetPortOperStatus) {
  PushChassisConfigSuccessfully();
...
TEST_F(IpdkSwitchTest, GetPortAdminStatus) {
  PushChassisConfigSuccessfully();
...
TEST_F(IpdkSwitchTest, GetMacAddressPass) {
  WriterMock<DataResponse> writer;
  DataResponse resp;
...
#if 0
// No BcmPort
TEST_F(IpdkSwitchTest, GetPortSpeed) {
  PushChassisConfigSuccessfully();
...
TEST_F(IpdkSwitchTest, GetMemoryErrorAlarmStatePass) {
  WriterMock<DataResponse> writer;
  DataResponse resp;
#if 0
  // Expect Write() call and store data in resp.
...
TEST_F(IpdkSwitchTest, GetFlowProgrammingExceptionAlarmStatePass) {
  WriterMock<DataResponse> writer;
  DataResponse resp;
  // Expect Write() call and store data in resp.
  ExpectMockWriteDataResponse(&writer, &resp);
...
// Doesn't work
TEST_F(IpdkSwitchTest, GetHealthIndicatorPass) {
  WriterMock<DataResponse> writer;
  DataResponse resp;
  // Expect Write() call and store data in resp.
  ExpectMockWriteDataResponse(&writer, &resp);
...
// Doesn't work
TEST_F(IpdkSwitchTest, GetForwardingViablePass) {
  WriterMock<DataResponse> writer;
  DataResponse resp;
  // Expect Write() call and store data in resp.
  ExpectMockWriteDataResponse(&writer, &resp);
...
=
TEST_F(IpdkSwitchTest, GetQosQueueCountersPass) {
  WriterMock<DataResponse> writer;
  DataResponse resp;
  // Expect Write() call and store data in resp.
  ExpectMockWriteDataResponse(&writer, &resp);
...
TEST_F(IpdkSwitchTest, GetNodePacketIoDebugInfoPass) {
  WriterMock<DataResponse> writer;
  DataResponse resp;
  // Expect Write() call and store data in resp.
  ExpectMockWriteDataResponse(&writer, &resp);
...
TEST_F(IpdkSwitchTest, GetPortLoopbackStatus) {
  PushChassisConfigSuccessfully();
...
TEST_F(IpdkSwitchTest, GetSdnPortId) {
  PushChassisConfigSuccessfully();
...
TEST_F(IpdkSwitchTest, SetPortAdminStatusPass) {
  SetRequest req;
...
TEST_F(IpdkSwitchTest, SetPortLoopbackStatusPass) {
  EXPECT_CALL(*chassis_manager_mock_,
...
TEST_F(IpdkSwitchTest, SetPortMacAddressPass) {
  SetRequest req;
  auto* request = req.add_requests()->mutable_port();
  request->set_node_id(1);
  request->set_port_id(2);
...
TEST_F(IpdkSwitchTest, SetPortSpeedPass) {
  SetRequest req;
  auto* request = req.add_requests()->mutable_port();
  request->set_node_id(1);
  request->set_port_id(2);
...
TEST_F(IpdkSwitchTest, SetPortLacpSystemIdMacPass) {
  SetRequest req;
  auto* request = req.add_requests()->mutable_port();
  request->set_node_id(1);
  request->set_port_id(2);
...
TEST_F(IpdkSwitchTest, SetPortLacpSystemPriorityPass) {
  SetRequest req;
  auto* request = req.add_requests()->mutable_port();
  request->set_node_id(1);
  request->set_port_id(2);
...
TEST_F(IpdkSwitchTest, SetPortHealthIndicatorPass) {
  SetRequest req;
  auto* request = req.add_requests()->mutable_port();
  request->set_node_id(1);
  request->set_port_id(2);
...
TEST_F(IpdkSwitchTest, SetPortNoContentsPass) {
  SetRequest req;
  auto* request = req.add_requests()->mutable_port();
  request->set_node_id(1);
  request->set_port_id(2);
...
TEST_F(IpdkSwitchTest, SetNoContentsPass) {
  SetRequest req;
  req.add_requests();
  std::vector<::util::Status> details;
  EXPECT_OK(switch_->SetValue(/* node_id */ 0, req, &details));
