...
class IpdkHalTest : public ::testing::Test {
 protected:
  ...
  static ::testing::StrictMock<AuthPolicyCheckerMock>*
      auth_policy_checker_mock_;
  static IpdkHal* hal_;  // points to the singleton instance
};

constexpr char IpdkHalTest::kChassisConfigTemplate[];
constexpr char IpdkHalTest::kForwardingPipelineConfigsTemplate[];
constexpr char IpdkHalTest::kErrorMsg[];
constexpr uint64 IpdkHalTest::kNodeId1;
constexpr uint64 IpdkHalTest::kNodeId2;
constexpr int IpdkHalTest::kUnit1;
constexpr int IpdkHalTest::kUnit2;
constexpr OperationMode IpdkHalTest::kMode;

::testing::StrictMock<SwitchMock>* IpdkHalTest::switch_mock_ = nullptr;
::testing::StrictMock<AuthPolicyCheckerMock>*
    IpdkHalTest::auth_policy_checker_mock_ = nullptr;
IpdkHal* IpdkHalTest::hal_ = nullptr;

TEST_F(IpdkHalTest, SanityCheckFailureWhenExtURLsNotGiven) {
  FLAGS_external_stratum_urls = "";
...
TEST_F(IpdkHalTest, SanityCheckFailureWhenExtURLsAreInvalid) {
...
#if 0
TEST_F(IpdkHalTest, SanityCheckFailureWhenPersistentConfigDirFlagNotGiven) {
...
TEST_F(IpdkHalTest, ColdbootSetupSuccessForSavedConfigs) {
...
TEST_F(IpdkHalTest, ColdbootSetupSuccessForNoSavedConfigAtAll) {
...
TEST_F(IpdkHalTest, ColdbootSetupSuccessForNoForwardingPipelineConfig) {
...
TEST_F(IpdkHalTest, ColdbootSetupSuccessForNoChassisConfig) {
...
TEST_F(IpdkHalTest, ColdbootSetupFailureWhenChassisConfigPushFails) {
  // Setup and save the test config(s).
  ChassisConfig chassis_config;
  ForwardingPipelineConfigs forwarding_pipeline_configs;
  FillTestChassisConfigAndSave(&chassis_config);
...
TEST_F(IpdkHalTest, ColdbootSetupFailureWhenPipelineConfigPushFailsForSomeNodes) {
  // Setup and save the test config(s).
  ChassisConfig chassis_config;
  ForwardingPipelineConfigs forwarding_pipeline_configs;
  FillTestChassisConfigAndSave(&chassis_config);
...
TEST_F(IpdkHalTest, WarmbootSetupSuccessForSavedConfig) {
  // Setup and save the test config(s).
  ChassisConfig chassis_config;
  ForwardingPipelineConfigs forwarding_pipeline_configs;
  FillTestChassisConfigAndSave(&chassis_config);
...
TEST_F(IpdkHalTest, WarmbootbootSetupFailureForNoSavedConfig) {
...
TEST_F(IpdkHalTest, WarmbootSetupFailureWhenUnfreezeFails) {
  // Setup and save the test config(s).
  ChassisConfig chassis_config;
  ForwardingPipelineConfigs forwarding_pipeline_configs;
  FillTestChassisConfigAndSave(&chassis_config);
...
TEST_F(IpdkHalTest, ColdbootTeardownSuccess) {
...
TEST_F(IpdkHalTest, ColdbootTeardownFailureWhenSwitchInterfaceShutdownFails) {
...
TEST_F(IpdkHalTest, ColdbootTeardownFailureWhenAuthPolicyCheckerShutdownFails) {
  EXPECT_CALL(*switch_mock_, Shutdown()).WillOnce(Return(::util::OkStatus()));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
      .WillOnce(
          Return(::util::Status(StratumErrorSpace(), ERR_INTERNAL, kErrorMsg)));
...
TEST_F(IpdkHalTest, WarmbootTeardownSuccess) {
  EXPECT_CALL(*switch_mock_, Shutdown()).WillOnce(Return(::util::OkStatus()));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
      .WillOnce(Return(::util::OkStatus()));
...
TEST_F(IpdkHalTest, WarmbootTeardownFailure) {
  EXPECT_CALL(*switch_mock_, Shutdown())
      .WillOnce(
          Return(::util::Status(StratumErrorSpace(), ERR_INTERNAL, kErrorMsg)));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
...
void* TestShutdownThread(void* arg) {
  ...
  static_cast<IpdkHal*>(arg)->HandleSignal(SIGINT);
  return nullptr;
}
