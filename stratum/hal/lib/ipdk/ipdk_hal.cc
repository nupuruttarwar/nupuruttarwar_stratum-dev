...
void SignalRcvCallback(int value) {
  ...
  write(IpdkHal::pipe_write_fd_, &value, sizeof(value));
  errno = saved_errno;
}
...
IpdkHal* IpdkHal::singleton_ = nullptr;
ABSL_CONST_INIT absl::Mutex IpdkHal::init_lock_(absl::kConstInit);
int IpdkHal::pipe_read_fd_ = -1;
int IpdkHal::pipe_write_fd_ = -1;

IpdkHal::IpdkHal(OperationMode mode, SwitchInterface* switch_interface,
...
::util::Status IpdkHal::SanityCheck() {
...
::util::Status IpdkHal::Setup() {
    return Setup(FLAGS_warmboot);
}

::util::Status IpdkHal::Setup(bool warmboot) {
...
void IpdkHal::HandleSignal(int value) {
...
IpdkHal* IpdkHal::CreateSingleton(OperationMode mode,
                                  SwitchInterface* switch_interface,
                                  AuthPolicyChecker* auth_policy_checker) {
  absl::WriterMutexLock l(&init_lock_);
  if (!singleton_) {
    singleton_ = new IpdkHal(mode, switch_interface, auth_policy_checker);
...
::util::Status IpdkHal::InitializeServer() {
...
::util::Status IpdkHal::RegisterSignalHandlers() {
...
::util::Status IpdkHal::UnregisterSignalHandlers() {
...
void* IpdkHal::SignalWaiterThreadFunc(void*) {
  int signal_value;
  int ret = read(IpdkHal::pipe_read_fd_, &signal_value, sizeof(signal_value));
  ...
  IpdkHal* hal = IpdkHal::GetSingleton();
